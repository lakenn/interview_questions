...
class UnifiedDataAPI:
    ...
    def __init__(self, symbols: [str], start_time: TimeStamp):
        ...
        self.historical_data_cache = pd.DataFrame()
        self.live_data_queue = deque()

        self.historical_data_source = HistoricalDataSource()
        self.live_data_source = LiveDataSource(CURRENT_TIME - pd.Timedelta('2D') + pd.Timedelta('1ns'))

        # Pre-fetch historical data
        # self._load_historical_data()

        # Pre-fetch historical data using a generator pattern
        self.historical_data_iterator = self._load_historical_data()

        # Thread to handle live data asynchronously
        self.live_data_thread = Thread(target=self._start_live_data_stream)
        self.live_data_thread.start()

    def _fetch_historical_chunk(self, start_time: TimeStamp, end_time: TimeStamp) -> pd.DataFrame:
        """
        Fetch a chunk of historical data for a specific time range.
        """
        try:
            data = self.historical_data_source.get(self.symbols, start_time, end_time)
            if data is None or data.empty:
                raise ValueError(f"Received empty data for {start_time} to {end_time}")
            return data
        except Exception as e:
            logger.error(f"Error fetching historical data from {start_time} to {end_time}: {e}")
            return pd.DataFrame()  # Return an empty dataframe on failure

    def _load_historical_data(self):
        """
        Fetches historical data in parallel using concurrent requests.
        """
        chunk_size = pd.Timedelta("10D")
        current_time = self.start_time
        end_time = CURRENT_TIME - pd.Timedelta('2D')

        # List to hold the future objects for concurrent execution
        futures = []
        data_frames = []  # To store each chunk's DataFrame for concatenation

        with concurrent.futures.ThreadPoolExecutor() as executor:
            while current_time < end_time:
                chunk_end_time = min(current_time + chunk_size, end_time)
                futures.append(executor.submit(self._fetch_historical_chunk, current_time, chunk_end_time))
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                # Each chunk is a DataFrame; append to the list of DataFrames
                data_frames.append(historical_data)

            # Combine all the DataFrames into one
            combined_data = pd.concat(data_frames)

            # Sort by the timestamp to ensure chronological order
            combined_data.sort_values(by="timestamp", inplace=True)
            self.historical_data_cache = combined_data

            for timestamp, row in self.historical_data_cache.iterrows():
                yield timestamp, row

            # # Convert combined DataFrame into the desired format: (timestamp, data_point) tuples
            # for timestamp, row in combined_data.iterrows():
            #     data_point = row[self.symbols]  # Get the data for the symbols as a pd.Series
            #     self.historical_data_cache.append((timestamp, data_point))  # Store as (timestamp, data_point)

            # logger.info(f"Historical data loaded and cached: {len(self.historical_data_cache)} data points")

    def _start_live_data_stream(self):
        """
        Starts the live data stream by subscribing to the live data source.
        """
        def sink(timestamp: TimeStamp, data_point: pd.Series) -> None:
            """Handles each piece of live data and appends it to the live data queue."""
            self.live_data_queue.append((timestamp, data_point))

        try:
            # Subscribe to live data stream
            self.live_data_source.subscribe(self.symbols, sink)
        except Exception as e:
    ...
    def get_next_data_point(self) -> tuple[TimeStamp, pd.Series] | None:
        # your implementation here
        """
        Retrieves the next data point.
        """
        try:
            # Get the next historical data point if available
            timestamp, data_point = next(self.historical_data_iterator)
            return timestamp, data_point
        except StopIteration:
            # Start live subscription once historical data is exhausted
            if self.live_data_queue:
                timestamp, data_point = self.live_data_queue.popleft()  # Pop the next live data point
                return timestamp, data_point

            return None

    def stop(self):
        """Stop the live data stream gracefully."""
        self.live_data_source.stop()
        self.live_data_thread.join()  # Wait for the live data stream thread to finish
